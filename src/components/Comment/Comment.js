...
const Comment = ({ comment, userId, post_id, onCommentSubmit, onUpdate }) => {
	...
	const fetchLikes = useCallback(async () => {
		const response = await fetch(
			`http://localhost:8080/api/commentLikes/${comment.comment_id}`
		);
		if (response.ok) {
		...
		<Box bg="gray.100" p="4" rounded="md" mb="4" position="relative">
			...
			{isEditing ? (
				<>
					...
					/>
					<HStack spacing="4">
						<Button colorScheme="blue" size="sm" onClick={handleSaveEdit}>
							Save
						</Button>
			...
			{/* Like/Reply Buttons */}
			<HStack>
				<Button size="sm" variant="ghost" onClick={handleLikeComment}>
					❤️ {likeCount}
				</Button>
				<Button
					size="sm"
